...
<div>
    ...
    @if (!is_null($selectedCounty) && $selectedCounty != 0)
        <select class="select w-full px-6 py-2 my-1 text-sm font-semibold flex lg:inline-flex" name="city">
            <option value="0" class="block text-center px-3 text-sm leading-6 hidden">City</option>
            @foreach ($cities as $city)
                <option class="py-2 mt-2 absolute rounded-xl w-full z-50 overflow-auto" value="{{$city->id}}">
                    {{ucfirst($city->name)}}
                </option>
            @endforeach
        </select>
